...
function render() {
  ...
  grid.innerHTML = visible.length ? visible.map(item => `
    <a class="card" href="${escapeHTML(safeUrl(item.url))}" target="_blank" rel="noopener noreferrer">
      <div class="card-meta"><span class="source">${escapeHTML(item.source)}</span><time>${escapeHTML(item.kind || '招聘信息')} · ${displayTime(item.published)}</time></div>
...
function setupFilters() {
  ...
  sourceFilters.innerHTML = Object.entries(counts).map(([source, count]) => `<button class="filter" data-source="${escapeHTML(source)}">${escapeHTML(source)} <span>${count}</span></button>`).join('');
...
async function init() {
  try {
    ...
    document.querySelector('#articleCount').textContent = `${news.length} 条招聘`;
    document.querySelector('#updatedAt').textContent = `更新于 ${displayTime(data.generated_at)}`;
    setupFilters(); render();
  } catch (_) {
    grid.innerHTML = '<p class="empty">招聘数据暂时无法加载。请稍后刷新页面。</p>';
    document.querySelector('#editionDate').textContent = '招聘数据暂时不可用';
  }
}
init();
